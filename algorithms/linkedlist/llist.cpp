...
void llist::deleteIth(int I, el_t& OldNum){
  if (I > Count || I < 1){ //if position indicated is more than the nodes we have or less than 1
    throw OutOfRange();
  }else if(isEmpty()){ //calls isEmpty to check if list is empty before trying to remove
    throw Underflow();
  ...
  //to the one to be deleted through next
    Node *P = Front; //set node equal to front
    moveTo(I - 1, P); //move it to node preceeding index wanted
    Node *todelete = P -> Next; //set second node equal to firsts next
    P -> Next = P -> Next -> Next; //first nodes next is equal to two nodes over
    OldNum = todelete -> Elem; //setting oldnum in the elem
    delete todelete; //deleting the node to delete
    todelete = NULL; //set it to NULL
